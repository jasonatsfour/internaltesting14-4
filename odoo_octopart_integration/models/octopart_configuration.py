            ...
            try:
                ...
                if "errors" not in octopart_data:
                    for item in octopart_data:
                        all_offers = [list(map(lambda x: x['offers'], i)) for i in list(map(lambda y: y['sellers'], item['parts']))]
                        moq_list = [list(map(lambda x: x[0]['moq'], i)) for i in all_offers]

                        use_moq = self.is_less_than(list(itertools.chain.from_iterable(moq_list)), qty)
                        for offers in all_offers:
                            for offer in offers:
                                ...
                                if company_currency_qty:
                                    nearby_value = max(company_currency_qty)
